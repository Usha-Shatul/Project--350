...
async function loadDefaultPosts() {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/elections/posts/default`);
        if (!response.ok) {
        ...
        if (token && currentUser) {
            for (let election of elections) {
                try {
                    ...
                    const voteCheck = await fetch(`${API_BASE_URL}/elections/${election.id}/votes/check`, {
                    ...
                    const candidateCheck = await fetch(`${API_BASE_URL}/elections/${election.id}/candidates/check`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    ...
                    const voteReqCheck = await fetch(`${API_BASE_URL}/elections/${election.id}/vote-requests`, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });
                    if (voteReqCheck.ok) {
...
async function viewElectionDetails(electionId) {
    try {
        ...
        if (resultsPublished) {
            try {
                // Get stats
                const statsResponse = await fetch(`${API_BASE_URL}/elections/${electionId}/stats`, {
                ...
                // Get results
                const resultsResponse = await fetch(`${API_BASE_URL}/elections/${electionId}/results`, {
                    headers: token ? { 'Authorization': `Bearer ${token}` } : {}
                });
                ...
                for (const post of election.posts) {
                    ...
                    try {
                        const candidatesResponse = await fetch(
                            `${API_BASE_URL}/elections/${electionId}/posts/${post.id}/candidates`,
                            { headers: token ? { 'Authorization': `Bearer ${token}` } : {} }
                        );
...
async function requestVotePermission(electionId) {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/elections/${electionId}/vote-request`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
async function openVoteModal(electionId) {
    try {
        ...
        const electionResponse = await fetch(`${API_BASE_URL}/elections/${electionId}`);
        ...
        const requests = await fetch(`${API_BASE_URL}/elections/${electionId}/vote-requests`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
        ...
        if (!userRequest) {
            ...
            const reqResponse = await fetch(`${API_BASE_URL}/elections/${electionId}/vote-request`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
        ...
        const votesCheckResponse = await fetch(`${API_BASE_URL}/elections/${electionId}/votes/check`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function loadManagementVoters(electionId) {
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/${electionId}/vote-requests`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function approveCandidate(candidateId, electionId) {
    ...
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/candidates/${candidateId}/approve`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function rejectCandidate(candidateId, electionId) {
    ...
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/candidates/${candidateId}/reject`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function deleteCandidateConfirm(candidateId, electionId) {
    ...
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/candidates/${candidateId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
        });
...
async function approveVoter(requestId, electionId) {
    ...
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/vote-requests/${requestId}/approve`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error('Failed to approve');
...
async function rejectVoter(requestId, electionId) {
    ...
    try {
        const token = localStorage.getItem('token') || localStorage.getItem('Swe_Society_token');
        const response = await fetch(`${API_BASE_URL}/elections/vote-requests/${requestId}/reject`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!response.ok) throw new Error('Failed to reject');
...
async function loadVoterLogs(electionId) {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/elections/${electionId}/voter-logs`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
if (createBtn) {
    const updateButtonState = () => {
        ...
        createBtn.disabled = !isPanelMember;
        createBtn.style.opacity = isPanelMember ? '1' : '0.5';
        createBtn.style.cursor = isPanelMember ? 'pointer' : 'not-allowed';
    };
    ...
    createBtn.addEventListener('click', () => {
        ...
        if (!checkPanelMember(currentUser)) {
            showAlert('❌ Only panel members can create elections', 'danger');
            return;
        }
