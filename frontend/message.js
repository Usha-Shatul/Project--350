(function() {
    ...
    async function loadAllData() {
        try {
            // Load DMs
            const dmResp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages`);
            const dmData = await dmResp.json();
            const conversations = dmResp.ok && dmData.success ? dmData.data : [];

            // Load groups
            const groupResp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups`);
            const groupData
            = await groupResp.json();
            ...
            if (groupId) {
                ...
                if (existingGroup) {
                    ...
                    try {
                        const detailsResp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${groupId}`);
    ...
    async function openConversation(c) {
        ...
        // Mark as read
        try {
            await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/${c.id}/read`, {
                method: 'POST'
            });
    ...
    async function refreshGroupMessages() {
        ...
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${currentGroup.id}/messages`);
    ...
    async function refreshMessages() {
        ...
        const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/${currentConvo.id}/messages`);
        const data = await resp.json();
        if (resp.ok && data.success) {
    ...
    async function loadSettings() {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/settings`);
            const data = await resp.json();
            if (resp.ok && data.success) {
    ...
    async function searchUsers(q) {
        const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/search?q=${encodeURIComponent(q)}`);
        const data = await resp.json();
        if (resp.ok && data.success) {
    ...
    function renderSearchResults(users) {
        ...
        users.forEach(u => {
            ...
            div.addEventListener('click', async () => {
                ...
                searchInput.value = '';
                try {
                    const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/start`, {
                        method: 'POST',
    ...
    function wireEvents() {
        ...
        muteBtn.addEventListener('click', async () => {
            ...
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/${currentConvo.id}/mute`, {
                method: 'POST',
    ...
    async function updateNavbarBadge() {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/messages/unread`);
            const data = await resp.json();
            if (resp.ok && data.success) {
    ...
    async function leaveGroup(groupId) {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${groupId}/leave`, {
                method: 'POST'
            });
            const data = await resp.json();
    ...
    function wireGroupEvents() {
        ...
        if (submitGroupBtn) {
            submitGroupBtn.addEventListener('click', async () => {
                ...
                try {
                    console.log('Sending request to:', `${API_BASE_URL}/groups`);
                    const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups`, {
                        method: 'POST',
        ...
        document.querySelectorAll('.role-card').forEach(card => {
            card.addEventListener('click', async function() {
                ...
                if (checkbox.checked) {
                    ...
                    if (wasChecked) {
                        try {
                            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/users/role/${role}`);
                            const data = await resp.json();
                            if (resp.ok && data.success) {
    ...
    async function loadAllUsers() {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/users`);
            const data = await resp.json();
            if (resp.ok && data.success) {
                allUsers = data.data;
                renderMembersList(allUsers, 'membersList', 'selectedCount');
    ...
    async function showGroupInfo(groupId) {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${groupId}`);
            const data = await resp.json();
            if (resp.ok && data.success) {
        ...
        try {
            ...
            if (resp.ok && data.success) {
                ...
                submitBtn.onclick = async () => {
                    ...
                    try {
                        const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${groupId}/members`, {
                            method: 'POST',
    ...
    async function removeMember(groupId, userId) {
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${groupId}/members/${userId}`, {
                method: 'DELETE'
            });
            const data = await resp.json();
            if (resp.ok && data.success) {
