(function() {
    ...
    async function addMembersToGroup() {
        ...
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${currentGroup.id}/members`, {
                method: 'POST',
                body: JSON.stringify({
                    memberIds: Array.from(selectedMembers)
                })
    ...
    async function refreshGroupDetails() {
        if (!currentGroup) return;
        try {
            const resp = await window.authManager.authenticatedFetch(`${API_BASE_URL}/groups/${currentGroup.id}`);
            const data = await resp.json();
            if (resp.ok && data.success) {
    ...
    async function removeMember(memberId) {
        ...
        try {
            const resp = await window.authManager.authenticatedFetch(
                `${API_BASE_URL}/groups/${currentGroup.id}/members/${memberId}`,
                { method: 'DELETE' }
            );
            const data = await resp.json();
            if (resp.ok && data.success) {
    ...
    async function deleteGroup() {
        ...
        try {
            const resp = await window.authManager.authenticatedFetch(
                `${API_BASE_URL}/groups/${currentGroup.id}`,
                { method: 'DELETE' }
            );
            const data = await resp.json();
            if (resp.ok && data.success) {
