...
try {
    ...
    authManager = {
        setAuthData: (token, userData) => {
            localStorage.setItem('Swe_Society_token', token);
            localStorage.setItem('Swe_Society_user', JSON.stringify(userData));
        }
    };
}
...
async function loadAvailableRoles() {
    try {
        ...
        if (container) {
            container.innerHTML = '<div class="roles-error">Failed to load roles</div>';
        }
    }
}
...
function renderRolesSelector() {
    ...
    for (const [category, roles] of Object.entries(availableRoles)) {
        ...
        roles.forEach(role => {
            html += `<label class="role-checkbox">
                        <input type="checkbox" value="${role.id}" class="role-input" data-role-name="${role.role_name}">
        ...
        input.addEventListener('change', (e) => {
            if (e.target.checked) {
                selectedRoles.push({
                    ...
                    name: e.target.dataset.roleName
                });
            } else {
                selectedRoles = selectedRoles.filter(r => r.id !== parseInt(e.target.value));
            }
        });
    });
}
